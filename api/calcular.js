...
export default async function handler(req, res) {
  ...
  if (!servico) {
    res.status(400).json({ error: 'Servico obrigatorio' });
    return;
  }

  const prompt = [
    'Você é especialista em tabela tempária automotiva brasileira.',
    '',
    'Veículo: ' + veiculo,
    'Serviço: ' + servico,
    ...
    '',
    'REGRAS: Use os tempos reais. valor_servico = horas_estimadas x ' + valorHora,
    '',
    'Responda APENAS com JSON válido:',
    '{"horas_estimadas":0.5,"valor_servico":50.00,"justificativa":"texto tecnico","servicos_relacionados":["s1","s2","s3"]}'
  ].join('\n');
